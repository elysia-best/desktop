...
export APPNAME=${APPNAME:-OpenList}
export EXECUTABLE_NAME=${EXECUTABLE_NAME:-openlist}
...
./linuxdeploy-squashfs-root/AppRun --desktop-file=${DESKTOP_FILE} --icon-file=usr/share/icons/hicolor/512x512/apps/${EXECUTABLE_NAME}.png --executable=usr/bin/${EXECUTABLE_NAME} --appdir=AppDir
...
./linuxdeploy-squashfs-root/AppRun --desktop-file=${DESKTOP_FILE} \
	...
	--icon-file=usr/share/icons/hicolor/512x512/apps/${EXECUTABLE_NAME}.png --executable=usr/bin/${EXECUTABLE_NAME} --appdir=AppDir --output appimage
